...
// auf dem Board
function displayTask(task, col){

	// Allgemeines Task Div
	// enthält alle anderen Divs (Titel, Beschreibung, Priorität, Typ, Bearbeiten, Löschen)
	...
	// setze Attribute für das neue Div-Element
	// für genauere Beschreibung siehe Allgemeines Task Div oben
	taskDescriptionDiv.setAttribute("id", "taskDescription_"+task.getTaskId());
	taskDescriptionDiv.setAttribute("name", "taskDescription_"+task.getTaskId);
	taskDescriptionDiv.setAttribute("class", "taskDescription");

	// Schreibe Beschreibung anhand der Objekteigenschaft in das Div
	taskDescriptionDiv.innerHTML = task.getDescription();
	...
	// Lege Allgemeines Div anhand der Objekteigenschaft in die richtige Spalte auf dem Board
	switch(col) {
    	  case '1':
            col_toDo.appendChild(taskDiv);
            break;
    	  case '2':
            col_inPr.appendChild(taskDiv);
            break;
    	  case '3':
            col_test.appendChild(taskDiv);
            break;
    	  case '4':
            col_done.appendChild(taskDiv);
            break;	
    	  default:
	}
}
